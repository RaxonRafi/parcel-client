...
import { useBlockUserMutation, useDeleteUserMutation, useUnblockUserMutation } from "@/redux/features/users/user.api";
import { useState } from "react";
import { toast } from "sonner";
import { DeleteConfirmation } from "@/components/DeleteConfirmation";
import { BlockConfirmation } from "@/components/blockConfirmation";
import { useBlockParcelMutation, useGetAllParcelsQuery } from "@/redux/features/parcel/parcel.api";
...
export function UserList() {
 ...
 const handleBlockParcel = async (trackId: string) => {

      const toastId = toast.loading("Blocking...");
      try {
        const res = await blockParcel(trackId).unwrap();
              ...
              <TableRow key={parcel._id}>
                ...
                <TableCell className="flex gap-x-5">     
                   {/* <DeleteConfirmation onConfirm={()=>handleDeleteUser(user._id)}>
                    ...
                    <BlockConfirmation onConfirm={() => handleBlockParcel(user._id)}>
